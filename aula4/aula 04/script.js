...
let createdAt = document.querySelectorAll('h6');
...
let listadeFelinos = [
    ...
    for (let i = 0; i < 6; i++) {
        titulo[i].innerText = listadeFelinos[i].title;
        imagem[i].src = listadeFelinos[i].imgUrl;
        descricao[i].innerText = listadeFelinos[i].description;
        createdAt[i].innerText = listadeFelinos[i].createdAt;
    }
     /* for (const itens of item) {
     var lista = document.getElementsByTagName('ul')[0];
     var itens = lista.getElementsByTagName('li')[j]
     lista.removeChild(itens[2])// Removendo determinado elemento
     } */
